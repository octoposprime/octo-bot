...
func init() {
	...
	flag.StringVar(&ifaceName, "iface", "enp0s3", "network interface name")
	flag.Parse()
}
func main() {
...
func readARP(handle *pcap.Handle, iface *net.Interface, stop chan struct{}) {
	...
	// Set to store seen MAC addresses
	...
	for {
		...
		case packet = <-in:
			...
			arp := arpLayer.(*layers.ARP)
			if arp.Operation != layers.ARPReply || bytes.Equal([]byte(iface.HardwareAddr), arp.SourceHwAddress) {
				// This is a packet I sent or not a reply.
				continue
			}

			mac := net.HardwareAddr(arp.SourceHwAddress).String()
			// Check if MAC address has been seen before
			if _, ok := seenMACs[mac]; !ok {
				// Print only if MAC address is not seen before
				fmt.Fprintf(w, "%v\t%v\n", net.IP(arp.SourceProtAddress), net.HardwareAddr(arp.SourceHwAddress))
				w.Flush()
				// Mark MAC address as seen
				seenMACs[mac] = struct{}{}
			}
		}
	}
